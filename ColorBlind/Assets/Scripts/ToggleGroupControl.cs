...
public class ToggleGroupControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Toggle first = toggles.FirstOrDefault((t) => t.GetComponent<ToggleValue>().Value == ToggleValue.ToggleValues.Easy);
        if (first != null)
        {
            first.isOn = true;
        }
    ...
    private void ToggleStateChanged(Toggle toggle, bool state)
    {
        ...
        if (state == true)
			manager.SetDifficultyLevel(toggle.GetComponent<ToggleValue>().Value);
    }
